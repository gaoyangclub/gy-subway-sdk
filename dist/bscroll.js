...
function initMixin(BScroll) {
  ...
  BScroll.prototype.setScale = function (scale) {
    ...
    this.scale = scale;
    // console.log("scale持续变化:" + scale);
    if (scale != preScale) {
      this.trigger("scaleChanged",scale);
    }
  };
...
function coreMixin(BScroll) {
  ...
  BScroll.prototype._animate = function (destX, destY, duration, easingFn) {
    ...
    console.log("scroll运动中  x:" + me.x + "  y:" + me.y + "   scale:" + me.scale);
...
function zoomMixin(BScroll) {
  ...
  BScroll.prototype._zoom = function (e) {
    ...
    if (scale < min) {
      scale = min;//0.5 * min * Math.pow(2.0, scale / min);
    } else if (scale > max) {
      scale = max;//2.0 * max * Math.pow(0.5, max / scale);
    }
